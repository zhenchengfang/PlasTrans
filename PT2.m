function PT2(SeqFile,ResultFile,t,tmp)
...
while p1<=num
    ...
    clear f
    PT([pwd,'/',tmp,'file',num2str(c),'.fna'],[pwd,'/',tmp,'out',num2str(c),'.csv'],t,tmp);
...
%         end
        ...
        if scores>=0.5
            Group(i,1).Possible_source='Transmissible';
        else
            Group(i,1).Possible_source='Non-transmissible';
%     elseif y==3
...
if size(ResultFile,2)<4 || ~strcmp(ResultFile(end-3:end),'.csv')
    ...
    disp([ResultFile,'.csv'])
    disp('Note: The current version of PlasTrans uses "Comma-Separated Values (CSV)" as the format of the output file!!')
    ResultFile=[ResultFile,'.csv'];
end
result=cell2table(result,'VariableNames',{'Header','Length','Score','Transmissibility'});
